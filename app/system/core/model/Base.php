...
abstract class Base {
    ...
    public static function find($id, \PDO $pdo = null, $forUpdate = FALSE) {
        ...
        $sql = "SELECT * FROM " . static::TABLE_NAME . " WHERE " . (empty(static::PRIMARY_KEY) ? 'id' : static::PRIMARY_KEY) . " = ?";
        if ($forUpdate) {
            $sql .= " FOR UPDATE";
        }
    ...
    public static function countBy($params = array(), \PDO $pdo = null, $flag = false) {
        ...
        $sql = "SELECT count( " . (true === $flag ? $id : "*") . " ) as count FROM " . static::TABLE_NAME . (true === $flag ? '' : $conditionSql);
