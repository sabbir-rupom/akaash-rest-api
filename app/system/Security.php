...
class Security
{
    ...
    public static function xssClean($str, $is_image = false)
    {
        ...
        foreach ($words as $word) {
            ...
            $str = preg_replace_callback('#(' . substr($word, 0, -3) . ')(\W)#is', [self::class, 'compactExplodedWords'], $str);
        }
        ...
        do {
            ...
            if (preg_match('/<a/i', $str)) {
                $str = preg_replace_callback('#<a(?:rea)?[^a-z0-9>]+([^>]*?)(?:>|$)#si', [self::class, 'jsLinkRemoval'], $str);
            }

            if (preg_match('/<img/i', $str)) {
                $str = preg_replace_callback('#<img[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#si', [self::class, 'jsImgRemoval'], $str);
            }
