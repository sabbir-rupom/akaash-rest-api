...
use Akaash\System\Cache\LocalCache;
...
class Config
{
    ...
    {
        if (array_key_exists("CHECK_REQUEST_TOKEN", $this->_config)
            && intval($this->_config["CHECK_REQUEST_TOKEN"]) > 0) {
            return true;
        }
        return false;
    }
    ...
    {
        if (array_key_exists('SERVER_CACHE_ENABLE_FLAG', $this->_config)
            && intval($this->_config["SERVER_CACHE_ENABLE_FLAG"]) > 0) {
            return true;
        }
        return false;
    }
    ...
    public function cacheService()
    {
        if ($this->isLocalFileCacheEnable()) {
            return new LocalCache($this->getLocalCachePath());
        } else {
            return new Memcached($this->getMemcacheHost(), $this->getMemcachePort());
        }
    }
