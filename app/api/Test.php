...
class Test extends BaseClass {
    ...
    public function action() {
         ...
         */
        if (Config::getInstance()->checkRequestTokenFlag() === FALSE) {
            ...
            if (empty($this->config['REQUEST_TOKEN_SECRET'])) {
                ...
                $result = JwtToken::createToken(array('test' => 1), $this->config['REQUEST_TOKEN_SECRET']);
                if ($result['error'] == 0) {
                    $result = JwtToken::verifyToken($result['token'], $this->config['REQUEST_TOKEN_SECRET']);
                }
         ...
         */
        $checkLogStatus = TRUE;
        if (Config::getInstance()->isLogEnable()) {
        ...
        if (Config::getInstance()->isCacheEnable()) {
            if (Config::getInstance()->isLocalCache()) {
                /*
                 * Check local cache path access permission
                 */
                $cachePath = Config::getInstance()->getLocalCachePath();
            ...
            if ($cacheSystemStatus) {
                 ...
                 */
                $memcache->flush();
                $memcache->set($key, 'Test Data', MEMCACHE_COMPRESSED, 120);  // set sample cache data
        ...
        $profileImagePath = UPLOAD_PROFILE_IMAGE_PATH;
        if (!file_exists($profileImagePath) && !is_dir($profileImagePath)) {
            ...
            if (!is_writable($profileImagePath)) {
                ...
                $responseArray['testImagUrl'] = SERVER_HOST . "/uploads/profile_images/" . FileUpload::processBinaryImage('test', $imgBase64Data, 'profile', TRUE);
